...
class ASK14_nga:
    ...
    def __init__(self):
        ...
        self.regions = ['CA','TW','CN','JP',]    # region for regional corrections (Vs30 and Rrup)
    ...
    def __call__(self,M,Rjb,Vs30,T,rake, Ftype=None, \
                 Rrup=None,Rx=None,Ry0=None, dip=None,Ztor=None,Z10=None,\
                 W=None,Zhypo=None,azimuth=None,Fhw=None, \
                 Fas=0, CRjb=15, VsFlag=1, region='CA', country='California', \
    ...
    def hw_function(self,Tother=None):
        ...
        if self.Rx < 0:
            ...
            # taper3 (constrain the hanging wall effects, this should decreasing the difference between simulation and NGA GMPEs)
    ...
    def calc_MeanZ10(self,Vs30=None, country='California'):
        ...
        if country == 'Japan':
            lnZ10 = -5.23/2. * np.log((Vs30**2+412.**2)/(1360.**2+412.**2))
        else:
            lnZ10 = -7.67/4. * np.log((Vs30**4+610.**4)/(1360.**4+610.**4))
    ...
    def calc_sigma_tau(self):
        ...
        if self.country != 'Japan':
            ...
            phi_AL = s1*(self.M<4) + (s1+(s2-s1)/2*(self.M-4))*(4<=self.M<=6) + s2*(self.M>6)
        else:
            s5 = self.Coefs[Ti]['s5']
            s6 = self.Coefs[Ti]['s6']
            phi_AL = s5*(self.Rrup<30) + (s5+(s6-s5)/50.*(self.Rrup-30))*(30<=self.Rrup<=80) + s6*(self.Rrup>80)

        s3 = self.Coefs[Ti]['s3']
        s4 = self.Coefs[Ti]['s4']
        tau_AL = s3*(self.M<5) + (s3+(s4-s3)/2.*(self.M-5))*(5<=self.M<7) + s4*(self.M>=7)
